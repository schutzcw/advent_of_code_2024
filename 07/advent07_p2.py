...
def possible_equation(result: int, inputs: list[int]) -> bool:
	...
	# concat
	concat = int(str(inputs[0]) + str(inputs[1]))
	return possible_equation(result, [multiply] + inputs[2:]) or possible_equation(result, [add] + inputs[2:]) or possible_equation(result, [concat] + inputs[2:])
